...
SUPER_VERSION="5.1.0-rc4"
readonly SUPER_VERSION
SUPER_DATE="2026/02/20"
readonly SUPER_DATE
...
show_usage() {
  ...
  [--install-macos-bsi-updates]  [--install-macos-bsi-updates-off]
  ...
  [--install-safari-updates-without-restarting]
  [--install-safari-updates-without-restarting-off]
  ...
  [--workflow-ignore-managed-deferrals]
  [--workflow-ignore-managed-deferrals-off]
  ...
  ** Managed preferences override local options via domain: com.macjutsu.super
  ...
  <key>ConfigCodeSignature</key> <string>Identifier</string>
...
show_config_status() {
	...
	if [[ -n "${alternate_temporary_plist}" ]]; then
		if [[ -f "${alternate_temporary_plist}.plist" ]]; then
			...
			echo -e "$(get_config_code_signature "${alternate_temporary_plist}.plist")"
			{ [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ "${config_temp_override}" == "TRUE" ]]; } && echo -e "\n**** Config Status: Contents of the CONFIGURATION PROFILE MANAGED super settings file located at ${SUPER_MANAGED_PLIST}.plist:"
	...
	if [[ -n "${alternate_default_plist}" ]]; then
		if [[ -f "${alternate_default_plist}.plist" ]]; then
			...
			echo -e "$(get_config_code_signature "${alternate_default_plist}.plist")"
		else
	...
	if [[ $(ls "${SUPER_CONFIGS_FOLDER}") ]]; then
		for super_inactive_alt_plist in "${SUPER_CONFIGS_FOLDER:?}"/*.plist; do
			if [[ "${super_inactive_alt_plist}" != "${alternate_temporary_plist}.plist" ]] && [[ "${super_inactive_alt_plist}" != "${alternate_default_plist}.plist" ]]; then
				...
				echo -e "$(get_config_code_signature "${super_inactive_alt_plist}")"
			fi
		done
	fi
...
set_defaults() {
	...
	# Path to the log for the super metrics log:
	SUPER_METRICS_LOG="${SUPER_LOG_FOLDER}/super-metrics.log"
	readonly SUPER_METRICS_LOG
	...
	IBM_NOTIFIER_DOWNLOAD_URL="https://github.com/IBM/mac-ibm-notifications/releases/download/v-3.2.3-b-135/IBM.Notifier.zip"
	readonly IBM_NOTIFIER_DOWNLOAD_URL
	
	# Target version for IBM Notifier.app:
	IBM_NOTIFIER_TARGET_VERSION="3.2.3"
...
get_options() {
	...
	while [[ -n "$1" ]]; do
		...
		--install-macos-bsi-updates)
			install_macos_bsi_updates_option="TRUE"
			;;
		--install-macos-bsi-updates-off)
			install_macos_bsi_updates_option="FALSE"
			;;
		...
		--install-non-system-updates-without-restarting-off)
			...
			;;
		--install-safari-updates-without-restarting)
			install_safari_updates_without_restarting_option="TRUE"
			;;
		--install-safari-updates-without-restarting-off)
			install_safari_updates_without_restarting_option="FALSE"
			;;
		--install-prioritize-non-restart-updates)
...
get_pref_managed() {
	...
	if [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ -n "${alternate_temporary_plist}" ]] && [[ "${config_temp_override}" == "TRUE" ]]; then
		...
		[[ -n "${preference_result}" ]] && preference_source="${super_alternate_plist}"
	fi
	if [[ -f "${SUPER_MANAGED_PLIST}.plist" ]] && [[ -z "${preference_result}" ]]; then
		preference_result=$(defaults read "${SUPER_MANAGED_PLIST}" "${1}" 2>/dev/null)
		[[ -n "${preference_result}" ]] && preference_source="${SUPER_MANAGED_PLIST}"
	fi
	if [[ -n "${preference_result}" ]]; then
		local preference_type
		preference_type=$(defaults read-type "${preference_source}" "${1}" 2>/dev/null)
...
# Validate the code signing signature of the specified ${1} plist and return "UNSIGNED", "INVALID", or the output the valid code signature details.
get_config_code_signature(){
	local codesign_verify_response
	codesign_verify_response=$(codesign --verify -v "${1}" 2>&1)
	[[ $(echo "${codesign_verify_response}" | grep -c 'not signed') -gt 0 ]] && echo -e "** UNSIGNED **" && return 0
	[[ $(echo "${codesign_verify_response}" | grep -c 'invalid') -gt 0 ]] && echo -e "** INVALID CODE SIGNATURE **\n${codesign_verify_response}"  && return 0
	[[ $(echo "${codesign_verify_response}" | grep -c 'valid') -gt 0 ]] && echo -e "** VALID CODE SIGNATURE **\n$(codesign --display -v "${1}" 2>&1)"
}
...
manage_parameter_options() {
	...
	config_code_signature=$(get_pref_managed "ConfigCodeSignature")
	[[ -z "${config_code_signature}" ]] && config_code_signature="FALSE"
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: config_code_signature is: ${config_code_signature}"
	...
	if [[ "${config_edit_main_option}" != "TRUE" ]] && [[ -z "${config_edit_option}" ]] && [[ -z "${config_delete_option}" ]] && [[ -z "${config_delete_all_option}" ]]; then
		if [[ "${config_start_default_option}" == "X" ]]; then
			...
			if [[ -n "${unset_alternate_plist}" ]]; then
				delete_pref_main "AlternateDefaultPlist"
				delete_pref_main "AlternateDefaultPlistStatus"
		...
		if [[ "${config_start_temp_option}" == "X" ]]; then
			...
			if [[ -n "${unset_alternate_plist}" ]]; then
				delete_pref_main "AlternateTemporaryPlist"
				delete_pref_main "AlternateTemporaryPlistStatus"
		...
		if [[ -n "${alternate_default_plist}" ]]; then
			if [[ -f "${alternate_default_plist}.plist" ]]; then
				get_config_code_signature_response=$(get_config_code_signature "${alternate_default_plist}.plist")
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_config_code_signature_response is:\n${get_config_code_signature_response}"
				if [[ $(echo "${get_config_code_signature_response}" | grep -c ' UNSIGNED') -gt 0 ]] || [[ $(echo "${get_config_code_signature_response}" | grep -c ' VALID') -gt 0 ]]; then
					if [[ "${config_code_signature}" == "FALSE" ]] || [[ $(echo "${get_config_code_signature_response}" | grep -c "${config_code_signature}") -gt 0 ]]; then
						set_pref_main "AlternateDefaultPlistStatus" "VALID"
					else
						log_super "Parameter Error: The default workflow configuration at ${alternate_default_plist}.plist does not match the code signature requirement of ${config_code_signature}."
						set_pref_main "AlternateDefaultPlistStatus" "UNMATCHED"
						parameter_error="TRUE"
					fi
				else
					log_super "Parameter Error: The default workflow configuration at ${alternate_default_plist}.plist has an invalid code signature."
					set_pref_main "AlternateDefaultPlistStatus" "INVALID"
					parameter_error="TRUE"
				fi
				unset get_config_code_signature_response
			else
				log_super "Parameter Error: Unable to locate default workflow configuration at ${alternate_default_plist}.plist."
				set_pref_main "AlternateDefaultPlistStatus" "MISSING"
				parameter_error="TRUE"
			fi
		fi
		...
		if [[ -n "${alternate_temporary_plist}" ]]; then
			if [[ -f "${alternate_temporary_plist}.plist" ]]; then
				get_config_code_signature_response=$(get_config_code_signature "${alternate_temporary_plist}.plist")
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: get_config_code_signature_response is:\n${get_config_code_signature_response}"
				if [[ $(echo "${get_config_code_signature_response}" | grep -c ' UNSIGNED') -gt 0 ]] || [[ $(echo "${get_config_code_signature_response}" | grep -c ' VALID') -gt 0 ]]; then
					if [[ "${config_code_signature}" == "FALSE" ]] || [[ $(echo "${get_config_code_signature_response}" | grep -c "${config_code_signature}") -gt 0 ]]; then
						set_pref_main "AlternateTemporaryPlistStatus" "VALID"
					else
						log_super "Parameter Error: The default workflow configuration at ${alternate_default_plist}.plist does not match the code signature requirement of ${config_code_signature}."
						set_pref_main "AlternateTemporaryPlistStatus" "UNMATCHED"
						parameter_error="TRUE"
					fi
				else
					log_super "Parameter Error: The temporary workflow configuration at ${alternate_temporary_plist}.plist has an invalid code signature."
					set_pref_main "AlternateTemporaryPlistStatus" "INVALID"
					parameter_error="TRUE"
				fi
				unset get_config_code_signature_response
			else
				log_super "Parameter Error: Unable to locate temporary workflow configuration at ${alternate_temporary_plist}.plist."
				set_pref_main "AlternateTemporaryPlistStatus" "MISSING"
				parameter_error="TRUE"
			fi
		fi
		[[ "${parameter_error}" == "TRUE" ]] && return 0
	...
	if [[ "${install_macos_minor_version_target}" == "X" ]] || { [[ -z "${install_macos_minor_version_target}" ]] && [[ "${install_macos_minor_version_target_option}" == "X" ]]; }; then
	...
	{ [[ -n "${install_macos_minor_version_target}" ]] && [[ ! "${install_macos_minor_version_target}" =~ ${REGEX_MACOS_TARGET_VERSION} ]]; } && log_super "Parameter Error: The --install-macos-minor-version-target=number value must be a contemporary macOS version number (15.0 | 15.1 | 15.1.1)." && parameter_error="TRUE"
	{ [[ -n "${install_macos_minor_version_target}" ]] && [[ $(echo "${install_macos_minor_version_target}" | grep -c '\.') -eq 0 ]]; } && log_super "Parameter Error: The --install-macos-minor-version-target=number value must inlucde macOS major and minor version numbers (15.0 | 15.1 | 15.1.1)." && parameter_error="TRUE"
	...
	if [[ "${install_macos_major_version_target}" == "X" ]] || { [[ -z "${install_macos_major_version_target}" ]] && [[ "${install_macos_major_version_target_option}" == "X" ]]; }; then
		delete_pref "InstallMacOSMajorVersionTarget"
		unset install_macos_major_version_target
	...
	# Manage the ${install_macos_bsi_updates} and ${install_rapid_security_responses} parameters.
	install_macos_bsi_updates=$(get_pref_managed "InstallMacOSBSIUpdates")
	{ [[ -z "${install_macos_bsi_updates}" ]] && [[ -n "${install_macos_bsi_updates_option}" ]]; } && install_macos_bsi_updates="${install_macos_bsi_updates_option}" && set_pref "InstallMacOSBSIUpdates" "${install_macos_bsi_updates_option}"
	[[ -z "${install_macos_bsi_updates}" ]] && install_macos_bsi_updates=$(get_pref "InstallMacOSBSIUpdates")
	...
	if [[ -n "${install_rapid_security_responses}" ]]; then
		log_super "Parameter Warning: The --install-rapid-security-responses option is deprecated and will be removed in a future version of super. It's superseded by the --install-macos-bsi-updates option."
		[[ -z "${install_macos_bsi_updates}" ]] && install_macos_bsi_updates="${install_rapid_security_responses}"
		unset install_rapid_security_responses
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_macos_bsi_updates}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_bsi_updates is: ${install_macos_bsi_updates}"
	...
	# Manage ${install_safari_updates_without_restarting} parameter.
	install_safari_updates_without_restarting=$(get_pref_managed "InstallSafariUpdatesWithoutRestarting")
	{ [[ -z "${install_safari_updates_without_restarting}" ]] && [[ -n "${install_safari_updates_without_restarting_option}" ]]; } && install_safari_updates_without_restarting="${install_safari_updates_without_restarting_option}" && set_pref "InstallSafariUpdatesWithoutRestarting" "${install_safari_updates_without_restarting_option}"
	[[ -z "${install_safari_updates_without_restarting}" ]] && install_safari_updates_without_restarting=$(get_pref "InstallSafariUpdatesWithoutRestarting")
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${install_safari_updates_without_restarting}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_safari_updates_without_restarting is: ${install_safari_updates_without_restarting}"
	...
	if [[ "${install_jamf_policy_triggers}" == "X" ]] || { [[ -z "${install_jamf_policy_triggers}" ]] && [[ "${install_jamf_policy_triggers_option}" == "X" ]]; }; then
	...
	if [[ "${deferral_timer_minutes}" == "X" ]] || { [[ -z $deferral_timer_minutes ]] && [[ "${deferral_timer_default_option}" == "X" ]]; }; then
		delete_pref "DeferralTimerDefault"
		unset deferral_timer_minutes
	...
	if [[ "${deferral_timer_menu_minutes}" == "X" ]] || { [[ -z "${deferral_timer_menu_minutes}" ]] && [[ "${deferral_timer_menu_option}" == "X" ]]; }; then
	...
	if [[ "${deferral_timer_focus_minutes}" == "X" ]] || { [[ -z $deferral_timer_focus_minutes ]] && [[ "${deferral_timer_focus_option}" == "X" ]]; } then
		delete_pref "DeferralTimerFocus"
		unset deferral_timer_focus_minutes
	...
	if [[ "${deferral_timer_error_minutes}" == "X" ]] || { [[ -z $deferral_timer_error_minutes ]] && [[ "${deferral_timer_error_option}" == "X" ]]; }; then
		delete_pref "DeferralTimerError"
		unset deferral_timer_error_minutes
	...
	if [[ "${deferral_timer_workflow_relaunch_minutes}" == "X" ]] || { [[ -z $deferral_timer_workflow_relaunch_minutes ]] && [[ "${deferral_timer_workflow_relaunch_option}" == "X" ]]; }; then
		delete_pref "DeferralTimerWorkflowRelaunch"
		unset deferral_timer_workflow_relaunch_minutes
	...
	if [[ "${schedule_workflow_active}" == "X" ]] || { [[ -z "${schedule_workflow_active}" ]] && [[ "${schedule_workflow_active_option}" == "X" ]]; }; then
	...
	if [[ "${schedule_deferred_start_file}" == "X" ]] || { [[ -z "${schedule_deferred_start_file}" ]] && [[ "${schedule_deferred_start_file_option}" == "X" ]]; }; then
		delete_pref "ScheduleDeferredStartFile"
		unset schedule_deferred_start_file
	...
	if [[ "${schedule_deferred_start_days}" == "X" ]] || { [[ -z $schedule_deferred_start_days ]] && [[ "${schedule_deferred_start_days_option}" == "X" ]]; }; then
	...
	if [[ "${sofa_macos_url}" == "X" ]] || { [[ -z "${sofa_macos_url}" ]] && [[ "${schedule_zero_date_sofa_custom_url_option}" == "X" ]]; }; then
		delete_pref "ScheduleZeroDateSOFACustomURL"
		unset sofa_macos_url
	...
	if [[ "${schedule_zero_date_manual}" == "X" ]] || { [[ -z "${schedule_zero_date_manual}" ]] && [[ "${schedule_zero_date_manual_option}" == "X" ]]; }; then
		delete_pref "ScheduleZeroDateManual"
		unset schedule_zero_date_manual
	...
	if [[ "${scheduled_install_days}" == "X" ]] || { [[ -z "${scheduled_install_days}" ]] && [[ "${scheduled_install_days_option}" == "X" ]]; }; then
		delete_pref "ScheduledInstallDays"
		unset scheduled_install_days
	...
	if [[ "${scheduled_install_date}" == "X" ]] || { [[ -z "${scheduled_install_date}" ]] && [[ "${scheduled_install_date_option}" == "X" ]]; }; then
		delete_pref "scheduled_install_date"
	...
	if [[ "${scheduled_install_reminder_minutes}" == "X" ]] || { [[ -z "${scheduled_install_reminder_minutes}" ]] && [[ "${scheduled_install_reminder_option}" == "X" ]]; }; then
		delete_pref "ScheduledInstallReminder"
		unset scheduled_install_reminder_minutes
	...
	if [[ "${deadline_count_focus}" == "X" ]] || { [[ -z "${deadline_count_focus}" ]] && [[ "${deadline_count_focus_option}" == "X" ]]; }; then
		delete_pref "DeadlineCountFocus"
		unset deadline_count_focus
	...
	if [[ "${deadline_count_soft}" == "X" ]] || { [[ -z "${deadline_count_soft}" ]] && [[ "${deadline_count_soft_option}" == "X" ]]; }; then
		delete_pref "DeadlineCountSoft"
		unset deadline_count_soft
	...
	if [[ "${deadline_count_hard}" == "X" ]] || { [[ -z "${deadline_count_hard}" ]] && [[ "${deadline_count_hard_option}" == "X" ]]; }; then
		delete_pref "DeadlineCountHard"
		unset deadline_count_hard
	...
	if [[ "${deadline_days_focus}" == "X" ]] || { [[ -z "${deadline_days_focus}" ]] && [[ "${deadline_days_focus_option}" == "X" ]]; }; then
		delete_pref "DeadlineDaysFocus"
		unset deadline_days_focus
	...
	if [[ "${deadline_days_soft}" == "X" ]] || { [[ -z "${deadline_days_soft}" ]] && [[ "${deadline_days_soft_option}" == "X" ]]; }; then
		delete_pref "DeadlineDaysSoft"
		unset deadline_days_soft
	...
	if [[ "${deadline_days_hard}" == "X" ]] || { [[ -z "${deadline_days_hard}" ]] && [[ "${deadline_days_hard_option}" == "X" ]]; }; then
		delete_pref "DeadlineDaysHard"
		unset deadline_days_hard
	...
	if [[ "${deadline_date_focus}" == "X" ]] || { [[ -z "${deadline_date_focus}" ]] && [[ "${deadline_date_focus_option}" == "X" ]]; }; then
		delete_pref "DeadlineDateFocus"
		unset deadline_date_focus
	...
	if [[ "${deadline_date_soft}" == "X" ]] || { [[ -z "${deadline_date_soft}" ]] && [[ "${deadline_date_soft_option}" == "X" ]]; }; then
		delete_pref "DeadlineDateSoft"
		unset deadline_date_soft
	...
	if [[ "${deadline_date_hard}" == "X" ]] || { [[ -z "${deadline_date_hard}" ]] && [[ "${deadline_date_hard_option}" == "X" ]]; }; then
		delete_pref "DeadlineDateHard"
		unset deadline_date_hard
	...
	if [[ "${dialog_timeout_default_seconds}" == "X" ]] || { [[ -z $dialog_timeout_default_seconds ]] && [[ "${dialog_timeout_default_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutDefault"
		unset dialog_timeout_default_seconds
	...
	if [[ "${dialog_timeout_user_auth_seconds}" == "X" ]] || { [[ -z $dialog_timeout_user_auth_seconds ]] && [[ "${dialog_timeout_user_auth_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutUserAuth"
		unset dialog_timeout_user_auth_seconds
	...
	if [[ "${dialog_timeout_user_choice_seconds}" == "X" ]] || { [[ -z $dialog_timeout_user_choice_seconds ]] && [[ "${dialog_timeout_user_choice_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutUserChoice"
		unset dialog_timeout_user_choice_seconds
	...
	if [[ "${dialog_timeout_user_schedule_seconds}" == "X" ]] || { [[ -z $dialog_timeout_user_schedule_seconds ]] && [[ "${dialog_timeout_user_schedule_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutUserSchedule"
		unset dialog_timeout_user_schedule_seconds
	...
	if [[ "${dialog_timeout_soft_deadline_seconds}" == "X" ]] || { [[ -z $dialog_timeout_soft_deadline_seconds ]] && [[ "${dialog_timeout_soft_deadline_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutSoftDeadline"
		unset dialog_timeout_soft_deadline_seconds
	...
	if [[ "${dialog_timeout_insufficient_storage_seconds}" == "X" ]] || { [[ -z $dialog_timeout_insufficient_storage_seconds ]] && [[ "${dialog_timeout_insufficient_storage_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutInsufficientStorage"
		unset dialog_timeout_insufficient_storage_seconds
	...
	if [[ "${dialog_timeout_power_required_seconds}" == "X" ]] || { [[ -z $dialog_timeout_power_required_seconds ]] && [[ "${dialog_timeout_power_required_option}" == "X" ]]; }; then
		delete_pref "DialogTimeoutPowerRequired"
		unset dialog_timeout_power_required_seconds
	...
	if [[ "${display_unmovable}" == "X" ]] || { [[ -z "${display_unmovable}" ]] && [[ "${display_unmovable_option}" == "X" ]]; }; then
	...
	if [[ "${display_hide_background}" == "X" ]] || { [[ -z "${display_hide_background}" ]] && [[ "${display_hide_background_option}" == "X" ]]; }; then
		delete_pref "DisplayHideBackground"
		unset display_hide_background
	...
	if [[ "${display_silently}" == "X" ]] || { [[ -z "${display_silently}" ]] && [[ "${display_silently_option}" == "X" ]]; }; then
		delete_pref "DisplaySilently"
		unset display_silently
	...
	# Validate ${display_hide_progress_bar_option} input and if valid set the ${display_hide_progress_bar} parameter.
	display_hide_progress_bar=$(get_pref_managed "DisplayHideProgressBar")
	if [[ "${display_hide_progress_bar}" == "X" ]] || { [[ -z "${display_hide_progress_bar}" ]] && [[ "${display_hide_progress_bar_option}" == "X" ]]; }; then
		delete_pref "DisplayHideProgressBar"
		unset display_hide_progress_bar
	elif [[ -z "${display_hide_progress_bar}" ]] && [[ -n "${display_hide_progress_bar_option}" ]]; then
		display_hide_progress_bar="${display_hide_progress_bar_option}"
		set_pref "DisplayHideProgressBar" "${display_hide_progress_bar}"
	fi
	[[ -z "${display_hide_progress_bar}" ]] && display_hide_progress_bar=$(get_pref "DisplayHideProgressBar")
	if [[ -n "${display_hide_progress_bar}" ]]; then
		previous_ifs="${IFS}"
		IFS=','
		local display_hide_progress_bar_array
		read -r -a display_hide_progress_bar_array <<<"${display_hide_progress_bar}"
		...
		IFS="${previous_ifs}"
		[[ $(echo "${display_hide_progress_bar}" | grep -c 'ALWAYS') -gt 0 ]] && display_hide_progress_bar_status="TRUE"
	fi
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_hide_progress_bar}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_hide_progress_bar is: ${display_hide_progress_bar}"
	{ [[ "${verbose_mode}" == "TRUE" ]] && [[ -n "${display_hide_progress_bar_status}" ]]; } && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: display_hide_progress_bar_status is: ${display_hide_progress_bar_status}"
	...
	if [[ "${display_notifications_centered}" == "X" ]] || { [[ -z "${display_notifications_centered}" ]] && [[ "${display_notifications_centered_option}" == "X" ]]; }; then
		delete_pref "DisplayNotificationsCentered"
		unset display_notifications_centered
	...
	if [[ "${display_icon_size}" == "X" ]] || { [[ -z $display_icon_size ]] && [[ "${display_icon_size_option}" == "X" ]]; }; then
		delete_pref "DisplayIconSize"
		unset display_icon_size
	...
	if [[ "${display_icon_file_path}" == "X" ]] || { [[ -z "${display_icon_file_path}" ]] && [[ "${display_icon_file_option}" == "X" ]]; }; then
		delete_pref "DisplayIconFile"
		delete_pref "DisplayIconLightFileCachedOrigin"
		delete_pref "DisplayIconDarkFileCachedOrigin"
		unset display_icon_file_path
		unset display_icon_light_file_cached_origin
		unset display_icon_dark_file_cached_origin
	...
	if [[ "${display_icon_light_file_path}" == "X" ]] || { [[ -z "${display_icon_light_file_path}" ]] && [[ "${display_icon_light_file_option}" == "X" ]]; }; then
		delete_pref "DisplayIconLightFile"
		delete_pref "DisplayIconLightFileCachedOrigin"
		unset display_icon_light_file_path
		unset display_icon_light_file_cached_origin
	...
	if [[ "${display_icon_dark_file_path}" == "X" ]] || { [[ -z "${display_icon_dark_file_path}" ]] && [[ "${display_icon_dark_file_option}" == "X" ]]; }; then
		delete_pref "DisplayIconDarkFile"
		delete_pref "DisplayIconDarkFileCachedOrigin"
		unset display_icon_dark_file_path
		unset display_icon_dark_file_cached_origin
	...
	if [[ "${display_accessory_type}" == "X" ]] || { [[ -z "${display_accessory_type}" ]] && [[ "${display_accessory_type_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryType"
		unset display_accessory_type
	...
	if [[ "${display_accessory_default}" == "X" ]] || { [[ -z "${display_accessory_default}" ]] && [[ "${display_accessory_default_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryDefaultFile"
		unset display_accessory_default
	...
	if [[ "${display_accessory_macos_minor_update}" == "X" ]] || { [[ -z "${display_accessory_macos_minor_update}" ]] && [[ "${display_accessory_macos_minor_update_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryMacOSMinorUpdateFile"
		unset display_accessory_macos_minor_update
	...
	if [[ "${display_accessory_macos_major_upgrade}" == "X" ]] || { [[ -z "${display_accessory_macos_major_upgrade}" ]] && [[ "${display_accessory_macos_major_upgrade_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryMacOSMajorUpgradeFile"
		unset display_accessory_macos_major_upgrade
	...
	if [[ "${display_accessory_non_system_updates}" == "X" ]] || { [[ -z "${display_accessory_non_system_updates}" ]] && [[ "${display_accessory_non_system_updates_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryNonSystemUpdatesFile"
		unset display_accessory_non_system_updates
	...
	if [[ "${display_accessory_safari_update}" == "X" ]] || { [[ -z "${display_accessory_safari_update}" ]] && [[ "${display_accessory_safari_update_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessorySafariUpdateFile"
		unset display_accessory_safari_update
	...
	if [[ "${display_accessory_jamf_policy_triggers}" == "X" ]] || { [[ -z "${display_accessory_jamf_policy_triggers}" ]] && [[ "${display_accessory_jamf_policy_triggers_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryJamfPolicyTriggersFile"
		unset display_accessory_jamf_policy_triggers
	...
	if [[ "${display_accessory_restart_without_updates}" == "X" ]] || { [[ -z "${display_accessory_restart_without_updates}" ]] && [[ "${display_accessory_restart_without_updates_file_option}" == "X" ]]; }; then
		delete_pref "DisplayAccessoryRestartWithoutUpdatesFile"
		unset display_accessory_restart_without_updates
	...
	if [[ "${display_help_button_string}" == "X" ]] || { [[ -z "${display_help_button_string}" ]] && [[ "${display_help_button_string_option}" == "X" ]]; }; then
		delete_pref "DisplayHelpButtonString"
		unset display_help_button_string
	...
	if [[ "${display_warning_button_string}" == "X" ]] || { [[ -z "${display_warning_button_string}" ]] && [[ "${display_warning_button_string_option}" == "X" ]]; }; then
		delete_pref "DisplayWarningButtonString"
		unset display_warning_button_string
	...
	if [[ "${mac_cpu_architecture}" == "arm64" ]]; then
		...
		if [[ "${auth_jamf_custom_url}" == "X" ]] || { [[ -z "${auth_jamf_custom_url}" ]] && [[ "${auth_jamf_custom_url_option}" == "X" ]]; }; then
			delete_pref_main "AuthJamfCustomURL"
			unset auth_jamf_custom_url
		...
		if [[ "${auth_mdm_failover_to_user}" == "X" ]] || { [[ -z "${auth_mdm_failover_to_user}" ]] && [[ "${auth_mdm_failover_to_user_option}" == "X" ]]; }; then
			delete_pref_main "AuthMDMFailoverToUser"
			unset auth_mdm_failover_to_user
	...
	if [[ "${test_mode_timeout_seconds}" == "X" ]] || { [[ -z $test_mode_timeout_seconds ]] && [[ "${test_mode_timeout_option}" == "X" ]]; }; then
		delete_pref "TestModeTimeout"
		unset test_mode_timeout_seconds
	...
	if [[ "${storage_required_update_gb}" == "X" ]] || { [[ -z "${storage_required_update_gb}" ]] && [[ "${test_storage_update_option}" == "X" ]]; }; then
		...
		unset storage_required_update_gb
	...
	if [[ "${storage_required_upgrade_gb}" == "X" ]] || { [[ -z "${storage_required_upgrade_gb}" ]] && [[ "${test_storage_upgrade_option}" == "X" ]]; }; then
		...
		unset storage_required_upgrade_gb
	...
	if [[ "${power_required_battery_percent}" == "X" ]] || { [[ -z "${power_required_battery_percent}" ]] && [[ "${test_battery_level_option}" == "X" ]]; }; then
		delete_pref "TestBatteryLevel"
		unset power_required_battery_percent
...
workflow_startup() {
	...
	if [[ "${open_logs_option}" == "TRUE" ]]; then
		if [[ "${current_user_account_name}" != "FALSE" ]]; then
			...
			sudo -u "${current_user_account_name}" open "${SUPER_METRICS_LOG}"
	...
	if [[ $(get_pref "WorkflowRestartValidate") == "TRUE" ]]; then
		...
		[[ $(echo "${display_hide_progress_bar}" | grep -c 'INSTALLNOW') -gt 0 ]] && display_hide_progress_bar_status="TRUE"
...
interactive_interrupt() {
	log_super "**** S.U.P.E.R.M.A.N. ${SUPER_VERSION} - INTERACTIVE INTERRUPT - PRESS ENTER TO CONTINUE OR CTRL-C TO EXIT ****"
...
# Append input to the log located at ${SUPER_METRICS_LOG} including an elapsed time based on the log text ${1} and timestamp ${2}. The timestamp can be either an integer of seconds, or a variable of a timestamp, or a property list key containing a timestamp.
log_metrics() {
	local log_metrics_elapsed_seconds
	local log_metrics_timestamp
	if [[ "${2}" =~ ^[0-9]+$ ]]; then
		log_metrics_elapsed_seconds="${2}"
	else # ${2} is not an integer.
		[[ ! "${2}" =~ ^[[:upper:]] ]] && log_metrics_timestamp="${2}"
		[[ "${2}" =~ ^[[:upper:]] ]] && log_metrics_timestamp=$(get_pref "${2}")
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: log_metrics_timestamp is: ${log_metrics_timestamp}"
		[[ -n "${log_metrics_timestamp}" ]] && log_metrics_elapsed_seconds=$(( $(date +%s) - $(date -j -f %Y-%m-%d:%H:%M:%S "${log_metrics_timestamp}" +%s) ))
	fi
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: log_metrics_elapsed_seconds is: ${log_metrics_elapsed_seconds}"
	if [[ -n "${log_metrics_elapsed_seconds}" ]]; then
		{ [[ -n "${workflow_target}" ]] && [[ "${workflow_target}" != "FALSE" ]]; } && echo -e "$(date +"%a %b %d %T") $(hostname -s) $(basename "$0")[$$]: ${workflow_target}: ${1} (DHMS): $(printf "%02d" $(( log_metrics_elapsed_seconds / 86400))):$(printf "%02d" $(( log_metrics_elapsed_seconds / 3600 % 24 ))):$(printf "%02d" $(( log_metrics_elapsed_seconds / 60 % 60 ))):$(printf "%02d" $(( log_metrics_elapsed_seconds %60 )))" >> "${SUPER_METRICS_LOG}"
		{ [[ -z "${workflow_target}" ]] || [[ "${workflow_target}" == "FALSE" ]]; } && echo -e "$(date +"%a %b %d %T") $(hostname -s) $(basename "$0")[$$]: No Target: ${1} (DHMS): $(printf "%02d" $(( log_metrics_elapsed_seconds / 86400))):$(printf "%02d" $(( log_metrics_elapsed_seconds / 3600 % 24 ))):$(printf "%02d" $(( log_metrics_elapsed_seconds / 60 % 60 ))):$(printf "%02d" $(( log_metrics_elapsed_seconds %60 )))" >> "${SUPER_METRICS_LOG}"
	else
		{ [[ -n "${workflow_target}" ]] && [[ "${workflow_target}" != "FALSE" ]]; } && echo -e "$(date +"%a %b %d %T") $(hostname -s) $(basename "$0")[$$]: ${workflow_target}: ${1}" >> "${SUPER_METRICS_LOG}"
		{ [[ -z "${workflow_target}" ]] || [[ "${workflow_target}" == "FALSE" ]]; } && echo -e "$(date +"%a %b %d %T") $(hostname -s) $(basename "$0")[$$]: No Target: ${1}" >> "${SUPER_METRICS_LOG}"
	fi
}
...
archive_logs() {
	...
	[[ $(ls -l "${SUPER_METRICS_LOG}" 2>/dev/null | awk '{print int($5/1000)}') -gt $SUPER_LOG_ARCHIVE_SIZE ]] && archive_logs_needed="TRUE"
	...
	if [[ "${archive_logs_needed}" == "TRUE" ]]; then
		...
		[[ -f "${SUPER_METRICS_LOG}" ]] && mv "${SUPER_METRICS_LOG}" "${SUPER_LOG_ARCHIVE_FOLDER}/${log_archive_name}/$(basename ${SUPER_METRICS_LOG})"
...
check_storage_available() {
	...
	if [[ -z "${storage_available_gigabytes}" ]] || [[ "${storage_available_gigabytes}" == "0" ]] || [[ ! "${storage_available_gigabytes}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
		log_super "Warning: Initial attempt to determine available storage returned an unexpected value of ${storage_available_gigabytes}, attempting fallback methods..."
		...
		if [[ -z "${storage_available_gigabytes}" ]] || [[ "${storage_available_gigabytes}" == "0" ]] || [[ ! "${storage_available_gigabytes}" =~ ${REGEX_ANY_WHOLE_NUMBER} ]]; then
			log_super "Warning: Further attempts to determine available storage returned an unexpected value of ${storage_available_gigabytes}, trying with another method..."
...
# Delete all workflow items, counters, and zero dates in ${SUPER_MAIN_PLIST} to reset the workflow target. Also update ${SUPER_METRICS_LOG}.
reset_workflow() {
	log_super "Status: Resetting workflow target."
	[[ -n "${workflow_target}" ]] && log_metrics "Workflow Complete: Total Elapsed Time" "WorkflowTargetTimestamp"
...
check_scheduled_install() {
	...
	if [[ "${workflow_scheduled_install_active}" == "TRUE" ]]; then
		...
		[[ $(echo "${display_hide_progress_bar}" | grep -c 'SCHEDULED') -gt 0 ]] && display_hide_progress_bar_status="TRUE"
	...
	if [[ "${user_focus_active}" == "TRUE" ]]; then
		if [[ -n "${deadline_count_focus}" ]]; then
			...
			if [[ $deadline_counter_focus_current -ge $deadline_count_focus ]]; then
				...
				log_metrics "Focus Count Expired: Workflow Elapsed Time" "WorkflowTargetTimestamp"
				...
				log_metrics "Focus Count Remaining ${display_string_deadline_count_focus}: Workflow Elapsed Time" "WorkflowTargetTimestamp"
			fi
		else
	...
	if [[ "${user_focus_active}" == "FALSE" ]]; then
		if [[ -n "${deadline_count_soft}" ]]; then
			...
			if [[ $deadline_counter_soft_current -ge $deadline_count_soft ]]; then
				...
				log_metrics "Soft Count Expired: Workflow Elapsed Time" "WorkflowTargetTimestamp"
				...
				log_metrics "Soft Count Remaining ${display_string_deadline_count_soft}: Workflow Elapsed Time" "WorkflowTargetTimestamp"
			fi
		...
		if [[ -n "${deadline_count_hard}" ]]; then
			...
			if [[ $deadline_counter_hard_current -ge $deadline_count_hard ]]; then
				...
				log_metrics "Hard Count Expired: Workflow Elapsed Time" "WorkflowTargetTimestamp"
				...
				log_metrics "Hard Count Remaining ${display_string_deadline_count_hard}: Workflow Elapsed Time" "WorkflowTargetTimestamp"
			fi
...
check_software_update_status_cached() {
	...
	local mac_last_startup_epoch
	mac_last_startup_epoch=$(date -j -u -f "%Y-%m-%d:%H:%M:%S" "${mac_last_startup}" +%s)
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mac_last_startup_epoch is: ${mac_last_startup_epoch}"
	if [[ $super_last_sccessful_check_epoch -lt $mac_last_startup_epoch ]]; then
		log_super "Status: Last macOS software status check was before last system startup time, full software status check required."
		check_software_status_required="TRUE"
		return 0
	fi
	...
	if [[ "${mdmclient_list_checksum_cache}" == "$(md5 -q "${MDMCLIENT_LIST_LOG}" 2>/dev/null)" ]]; then
		...
		if [[ -n "${mdmclient_available_cache}" ]]; then
			if [[ "${mdmclient_available_cache}" == "TRUE" ]]; then
				...
				mdmclient_list=$(sed -e '/^Available updates/q' < "${MDMCLIENT_LIST_LOG}")
			fi
		else
...
get_mdmclient_list() {
	...
	if [[ "${get_mdmclient_list_error}" == "FALSE" ]] && [[ "${get_mdmclient_list_timeout}" == "FALSE" ]]; then
		...
		if [[ $(echo "${mdmclient_list}" | grep -c 'OS Update Item') -gt 0 ]]; then
			mdmclient_available="TRUE"
...
# This function sets ${macos_msu_major_upgrade_target}, ${macos_installer_major_upgrade_target}, ${macos_msu_minor_update_target}, ${macos_installer_minor_update_target}, ${non_system_msu_targets}, and ${non_system_msu_safari_target}.
...
workflow_check_software_status() {
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]]; then
		...
		if [[ ${#mdmclient_macos_msu_array[@]} -eq 0 ]] && [[ ${#mdmclient_macos_installer_array[@]} -eq 0 ]] && [[ ${#mdmclient_non_system_array[@]} -eq 0 ]]; then
	...
	[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_installer_array is:\n${mdmclient_macos_installer_array[*]}"
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ ${#mdmclient_non_system_array[@]} -eq 0 ]] && [[ ${#mdmclient_macos_msu_array[@]} -eq 0 ]] && [[ ${#mdmclient_macos_installer_array[@]} -eq 0 ]]; then
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ "${install_macos_major_upgrades}" == "TRUE" ]] || [[ -n "${install_macos_major_version_target}" ]] || [[ -n "${install_macos_minor_version_target}" ]]; } && [[ ${#mdmclient_macos_installer_array[@]} -gt 0 ]]; then
		...
		# If there is a ${macos_installers_list} then parse items to create arrays..
		if [[ "${macos_installers_list}" == "FALSE" ]]; then
			...
			for macos_installer_item in "${macos_installers_array[@]}"; do
				...
				local macos_installer_item_version_normalized
				macos_installer_item_version_normalized=$(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item_version_normalized is: ${macos_installer_item_version_normalized}"
				...
				{ [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq $macos_version_major ]] && [[ $macos_installer_item_version_normalized -gt $macos_version_normalized ]]; } && macos_installer_minor_updates_array+=("${macos_installer_item}")
			done
	...
	# If (no errors) then evaluate versions in ${macos_msu_major_upgrade}, ${macos_installer_major_upgrades_array[]}, ${macos_msu_minor_update}, and ${macos_installer_minor_updates_array[]} for target selection. Also, log any available non-targetable macOS updates/upgrades.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ -n "${macos_msu_major_upgrade}" ]] || [[ ${#macos_installer_major_upgrades_array[@]} -gt 0 ]] || [[ -n "${macos_msu_minor_update}" ]] || [[ ${#macos_installer_minor_updates_array[@]} -gt 0 ]]; }; then
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: install_macos_major_upgrades is: ${install_macos_major_upgrades}"
		...
		# If ${install_macos_major_upgrades} then attempt to resolve a ${macos_msu_major_upgrade_target} given ${install_macos_major_version_target} and ${macos_msu_major_upgrade}.
		if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ -n "${macos_msu_major_upgrade}" ]]; then
			if [[ -z "${install_macos_major_version_target}" ]]; then
				macos_msu_major_upgrade_target="${macos_msu_major_upgrade}"
			else
				if [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${macos_msu_major_upgrade}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_major_version_target} ]]; then
					macos_msu_major_upgrade_target="${macos_msu_major_upgrade}"
				elif [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 1 ]] && [[ $(echo "${macos_msu_major_upgrade}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d", $1, $2}') -eq ${install_macos_major_version_normalized_target:0:4} ]]; then
					macos_msu_major_upgrade_target="${macos_msu_major_upgrade}"
				elif [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 2 ]] && [[ $(echo "${macos_msu_major_upgrade}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq ${install_macos_major_version_normalized_target} ]]; then
					macos_msu_major_upgrade_target="${macos_msu_major_upgrade}"
				else
			...
			if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] && [[ $macos_version_major -lt 13 ]] && [[ "${workflow_macos_auth}" == "JAMF" ]]; then
				log_super "Warning: The MDM upgrade command on macOS 12 only supports full macOS installers, thus the following \"over-the-air\" macOS major upgrade is not currently possible: ${macos_msu_major_upgrade_target}"
				macos_msu_major_upgrade_target="FALSE"
			fi
			if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]]; then
				macos_msu_major_upgrade_target_version_normalized=$(echo "${macos_msu_major_upgrade_target}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] && macos_major_upgrade_latest="TRUE"
				[[ $(echo "${mdmclient_macos_msu_array[*]}" | grep -c "$(echo "${macos_msu_major_upgrade_target}" | awk -F ':' '{print $5}')") -gt 0 ]] && macos_target_mdm_compatible="TRUE"
			fi
		fi
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_major_upgrade_target_version_normalized is: ${macos_msu_major_upgrade_target_version_normalized}"
		
		# If ${install_macos_major_upgrades} and there is no ${macos_msu_major_upgrade_target} then attempt to resolve a ${macos_installer_major_upgrade_target} given ${install_macos_major_version_target} and ${macos_installer_major_upgrades_array[]}.
		if [[ "${install_macos_major_upgrades}" == "TRUE" ]] && [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ ${#macos_installer_major_upgrades_array[@]} -gt 0 ]]; then
			if [[ -z "${install_macos_major_version_target}" ]]; then
				macos_installer_major_upgrade_target="${macos_installer_major_upgrades_array[0]}"
				[[ "${deferral_restrictions_macOS_major_upgrades}" == "FALSE" ]] && macos_major_upgrade_latest="TRUE"
			else
				for macos_installer_item in "${macos_installer_major_upgrades_array[@]}"; do
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item is: ${macos_installer_item}"
					if [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_major_version_target} ]]; then
						macos_installer_major_upgrade_target="${macos_installer_item}"
						break
					elif [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 1 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d", $1, $2}') -eq ${install_macos_major_version_normalized_target:0:4} ]]; then
						macos_installer_major_upgrade_target="${macos_installer_item}"
						break
					elif [[ $(echo "${install_macos_major_version_target}" | grep -o '\.' | wc -l | xargs) -eq 2 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq ${install_macos_major_version_normalized_target} ]]; then
						macos_installer_major_upgrade_target="${macos_installer_item}"
						break
					fi
				done
			...
			if [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]]; then
				macos_installer_major_upgrade_target_version_normalized=$(echo "${macos_installer_major_upgrade_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ $(echo "${mdmclient_macos_installer_array[*]}" | grep -c "$(echo "${macos_installer_major_upgrade_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g')") -gt 0 ]] && macos_target_mdm_compatible="TRUE"
			else
				log_super "Warning: Unable to target any available macOS major upgrade installer given the specified --install-macos-major-version-target option of ${install_macos_major_version_target}."
			fi
		fi
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_major_upgrade_target_version_normalized is: ${macos_installer_major_upgrade_target_version_normalized}"
		...
		# If ${install_macos_minor_updates} and there is no ${macos_msu_major_upgrade_target} or ${macos_installer_major_upgrade_target} then attempt to resolve a ${macos_msu_minor_update_target} target given ${install_macos_minor_version_target} and ${macos_msu_minor_update}.
		if [[ "${install_macos_minor_updates}" == "TRUE" ]] && [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && [[ -n "${macos_msu_minor_update}" ]]; then
			if [[ -z "${install_macos_minor_version_target}" ]]; then
				macos_msu_minor_update_target="${macos_msu_minor_update}"
			else
				if [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${macos_msu_minor_update}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_minor_version_target} ]]; then
					macos_msu_minor_update_target="${macos_msu_minor_update}"
				elif [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 1 ]] && [[ $(echo "${macos_msu_minor_update}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d", $1, $2}') -eq ${install_macos_minor_version_normalized_target:0:4} ]]; then
					macos_msu_minor_update_target="${macos_msu_minor_update}"
				elif [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 2 ]] && [[ $(echo "${macos_msu_minor_update}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq ${install_macos_minor_version_normalized_target} ]]; then
					macos_msu_minor_update_target="${macos_msu_minor_update}"
				else
			...
			{ [[ $(echo "${macos_msu_minor_update_target}" | grep -c '(') -gt 0 ]] && [[ "${install_macos_bsi_updates}" != "TRUE" ]]; } && log_super "Warning: The default workflow ignores macOS Background Security Improvement (BSI) updates. Use the --install-macos-bsi-updates option to always install macOS BSI updates as soon as they are available." && macos_msu_minor_update_target="FALSE"
			if [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; then
				macos_msu_minor_update_target_version_normalized=$(echo "${macos_msu_minor_update_target}" | awk -F ',' '{print $4}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ "${deferral_restrictions_macOS_minor_updates}" == "FALSE" ]] && macos_minor_update_latest="TRUE"
				[[ $(echo "${mdmclient_macos_msu_array[*]}" | grep -c "$(echo "${macos_msu_minor_update_target}" | awk -F ':' '{print $5}')") -gt 0 ]] && macos_target_mdm_compatible="TRUE"
			fi
		fi
		...
		if [[ "${install_macos_minor_updates}" == "TRUE" ]] && [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_msu_minor_update_target}" == "FALSE" ]] && [[ ${#macos_installer_minor_updates_array[@]} -gt 0 ]]; then
			if [[ -z "${install_macos_minor_version_target}" ]]; then
				...
				for macos_installer_item in "${macos_installer_minor_updates_array[@]}"; do
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item is: ${macos_installer_item}"
					if [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 0 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/\..*//g') -eq ${install_macos_minor_version_target} ]]; then
						macos_installer_minor_update_target="${macos_installer_item}"
						break
					elif [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 1 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d", $1, $2}') -eq ${install_macos_minor_version_normalized_target:0:4} ]]; then
						macos_installer_minor_update_target="${macos_installer_item}"
						break
					elif [[ $(echo "${install_macos_minor_version_target}" | grep -o '\.' | wc -l | xargs) -eq 2 ]] && [[ $(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') -eq ${install_macos_minor_version_normalized_target} ]]; then
						macos_installer_minor_update_target="${macos_installer_item}"
						break
					fi
				done
			fi
			if [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; then
				macos_installer_minor_update_target_version_normalized=$(echo "${macos_installer_minor_update_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ $(echo "${mdmclient_macos_installer_array[*]}" | grep -c "$(echo "${macos_installer_minor_update_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g')") -gt 0 ]] && macos_target_mdm_compatible="TRUE"
			else
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_minor_update_target_version_normalized is: ${macos_installer_minor_update_target_version_normalized}"
	...
	# If (no errors) then log any disallowed or inefficient macOS targets.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]]; then
		# Log disallowed macOS MSU items.
		if [[ ${#mdmclient_macos_msu_array[@]} -gt 0 ]]; then
			# Product Key:MSU_UPDATE_25C56_patch_26.2_major,Title:macOS Tahoe 26.2,Version:26.2
			local mdmclient_macos_item_version_normalized
			for mdmclient_macos_item in "${mdmclient_macos_msu_array[@]}"; do
		...
		#Log disallowed macOS installers.
		if [[ ${#macos_installer_major_upgrades_array[@]} -gt 0 ]] || [[ ${#macos_installer_minor_updates_array[@]} -gt 0 ]]; then
			# Title:macOS Tahoe,Version:26.2,Build:25C56,Size:17389952037
			for macos_installer_item in "${macos_installer_major_upgrades_array[@]}"; do
				macos_installer_item_version_normalized=$(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item_version_normalized is: ${macos_installer_item_version_normalized}"
				{ { [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]]; } || { [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] && [[ ${macos_installer_item_version_normalized} -gt ${macos_msu_major_upgrade_target_version_normalized} ]]; } || { [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] && [[ ${macos_installer_item_version_normalized} -gt ${macos_installer_major_upgrade_target_version_normalized} ]]; }; } && log_super "Disallowed: macOS major upgrade installer is available but not allowed: $(echo "${macos_installer_item}" | awk -F ',' '{print $1 $2}' | sed -e 's/Title://' -e 's/Version:/ /')"
			done
			if [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]]; then
				for macos_installer_item in "${macos_installer_minor_updates_array[@]}"; do
					macos_installer_item_version_normalized=$(echo "${macos_installer_item}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
					[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_installer_item_version_normalized is: ${macos_installer_item_version_normalized}"
					{ { [[ "${macos_msu_minor_update_target}" == "FALSE" ]] && [[ "${macos_installer_minor_update_target}" == "FALSE" ]]; } || { [[ "${macos_msu_minor_update_target}" != "FALSE" ]] && [[ ${macos_installer_item_version_normalized} -gt ${macos_msu_minor_update_target_version_normalized} ]]; } || { [[ "${macos_installer_minor_update_target}" != "FALSE" ]] && [[ ${macos_installer_item_version_normalized} -gt ${macos_installer_minor_update_target_version_normalized} ]]; }; } && log_super "Disallowed: macOS minor update installer is available but not allowed: $(echo "${macos_installer_item}" | awk -F ',' '{print $1 $2}' | sed -e 's/Title://' -e 's/Version:/ /')"
				done
			fi
		elif [[ ${#mdmclient_macos_installer_array[@]} -gt 0 ]] && [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_msu_minor_update_target}" == "FALSE" ]]; then
			# Product Key:093-37361,Title:macOS Tahoe,Version:26.2
			local mdmclient_macos_item_version_normalized
			for mdmclient_macos_item in "${mdmclient_macos_installer_array[@]}"; do
				mdmclient_macos_item_version_normalized=$(echo "${mdmclient_macos_item}" | awk -F ':' '{print $4}' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}')
				[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: mdmclient_macos_item_version_normalized is: ${mdmclient_macos_item_version_normalized}"
				[[ ${mdmclient_macos_item_version_normalized:0:2} -gt ${macos_version_major} ]] && { [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] || { [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] && [[ ${mdmclient_macos_item_version_normalized} -ne $(echo "${macos_installer_major_upgrade_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') ]]; }; } && log_super "Disallowed: macOS major upgrade installer is available but not allowed: $(echo "${mdmclient_macos_item}" | awk -F ',' '{print $2 $3}' | sed -e 's/Title://' -e 's/Version:/ /')"
				[[ ${mdmclient_macos_item_version_normalized:0:2} -eq ${macos_version_major} ]] && [[ "${macos_installer_major_upgrade_target}" == "FALSE" ]] && { [[ "${macos_installer_minor_update_target}" == "FALSE" ]] || { [[ "${macos_installer_minor_update_target}" != "FALSE" ]] && [[ ${mdmclient_macos_item_version_normalized} -ne $(echo "${macos_installer_major_upgrade_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' | awk -F'.' '{printf "%02d%02d%02d", $1, $2, $3}') ]]; }; } && log_super "Disallowed: macOS minor update installer is available but not allowed: $(echo "${mdmclient_macos_item}" | awk -F ',' '{print $2 $3}' | sed -e 's/Title://' -e 's/Version:/ /')"
			done
		fi
		
		# Log inefficient installers.
		{ [[ -n "${install_macos_minor_version_target}" ]] && [[ "${macos_msu_minor_update_target}" == "FALSE" ]] && [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; } && log_super "Warning: The --install-macos-minor-version-target=${install_macos_minor_version_target} option is forcing the workflow to target a full macOS installer (as opposed to a more efficient \"over-the-air\" macOS minor update)."
		{ [[ -n "${install_macos_major_version_target}" ]] && [[ "${macos_msu_major_upgrade_target}" == "FALSE" ]] && [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]]; } && log_super "Warning: The --install-macos-major-version-target=${install_macos_major_version_target} option is forcing the workflow to target a full macOS installer (as opposed to a more efficient \"over-the-air\" macOS major upgrade)."
	fi
	...
	# If (no errors) there is a ${macos_msu_major_upgrade_target} or ${macos_msu_minor_update_target} then set the remaining individual parameters.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; }; then
		local macos_msu_target
		[[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] && macos_msu_target="${macos_msu_major_upgrade_target}"
		[[ "${macos_msu_minor_update_target}" != "FALSE" ]] && macos_msu_target="${macos_msu_minor_update_target}"
		[[ $(echo "${macos_msu_target}" | grep -c 'Beta') -eq 0 ]] && macos_msu_title=$(echo "${macos_msu_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/ [1-9].*//g')
		[[ $(echo "${macos_msu_target}" | grep -c 'Beta') -gt 0 ]] && macos_msu_title=$(echo "${macos_msu_target}" | awk -F ',' '{print $2}' | sed -e 's/.*://g' -e 's/ [1-9].*/ Beta/g')
		macos_msu_version=$(echo "${macos_msu_target}" | awk -F ',' '{print $4}' | sed -e 's/.*://g')
		macos_msu_build=$(echo "${macos_msu_target}" | awk -F ',' '{print $1}' | sed -e 's/.*://g' -e 's/^.*-//g')
		macos_msu_size=$(echo "${macos_msu_target}" | awk -F ',' '{print $3}' | sed -e 's/.*://g' -e 's/[^0-9]//g' | awk '{print $1"/1000000 +1"}' | bc)
		[[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] && log_super "Target: \"Over-the-air\" macOS major upgrade ${macos_msu_title} ${macos_msu_version}-${macos_msu_build} which is a ${macos_msu_size}GB download."
		[[ "${macos_msu_minor_update_target}" != "FALSE" ]] && log_super "Target: \"Over-the-air\" macOS minor update ${macos_msu_title} ${macos_msu_version}-${macos_msu_build} which is a ${macos_msu_size}GB download."
		macos_msu_label=$(echo "${macos_msu_target}" | awk -F ',' '{print $1}' | sed -e 's/.*://g')
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_label is: ${macos_msu_label}"
	fi
	
	# If (no errors) there is a ${macos_installer_major_upgrade_target} or ${macos_installer_minor_update_target} then set the remaining individual parameters.
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && { [[ "${macos_installer_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_installer_minor_update_target}" != "FALSE" ]]; }; then
	...
	if [[ "${workflow_check_software_status_error}" == "FALSE" ]] && [[ ${#non_system_msu_array[@]} -gt 0 ]]; then
	...
	# Handle ${workflow_check_software_status_error} or complete the workflow.
	IFS="${previous_ifs}"
...
workflow_check_download_status() {
	...
	if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_msu_minor_update_target}" != "FALSE" ]]; then
		...
		if [[ "${macos_msu_downloaded_version}" != "FALSE" ]]; then
			if [[ "${macos_msu_downloaded_version}" == "${macos_msu_version}" ]] && [[ -z "${macos_msu_label_downloaded}" ]]; then
				log_metrics "MSU Download Complete: Workflow Elapsed Time" "WorkflowTargetTimestamp"
				macos_msu_label_downloaded="${macos_msu_label}" && set_pref_main "MacOSMSULabelDownloaded" "${macos_msu_label}"
			fi
		...
		[[ "${verbose_mode}" == "TRUE" ]] && log_super "Verbose Mode: Function ${FUNCNAME[0]}: Line ${LINENO}: macos_msu_automatic_download_ramped is: ${macos_msu_automatic_download_ramped}"
	fi
}
...
download_macos_installer() {
	...
	if [[ "${download_macos_installer_start_error}" == "FALSE" ]] || [[ "${download_macos_installer_start_timeout}" == "FALSE" ]] || [[ "${download_macos_installer_timeout_error}" == "FALSE" ]] || [[ "${download_macos_installer_app_error}" == "FALSE" ]] || [[ "${download_macos_installer_validation_error}" == "FALSE" ]]; then
		...
		log_metrics "Installer Download Complete: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		download_macos_installer_error="FALSE"
		macos_installer_download_required="FALSE"
		set_pref_main "MacOSInstallerDownloaded" "${macos_installer_title} ${macos_installer_version}-${macos_installer_build}"
	...
	if [[ "${macos_installer_download_required}" == "TRUE" ]] || [[ "${macos_msu_download_required}" == "TRUE" ]]; then
		...
		if [[ "${macos_msu_automatic_download_deferral}" == "TRUE" ]]; then
			if [[ "${workflow_install_now_active}" != "TRUE" ]] && [[ "${deadline_date_status}" == "FALSE" ]] && [[ "${deadline_days_status}" == "FALSE" ]] && [[ "${deadline_count_status}" == "FALSE" ]]; then
				if [[ "${macos_msu_automatic_download_ramped}" != "FALSE" ]]; then
					log_super "Warning: The target macOS minor update has not yet automatically downloaded to the system because it's being delayed by Apple's incremental rollout mechanism, trying again in ${deferral_timer_minutes} minutes."
					log_status "Pending: The target macOS minor update has not yet automatically downloaded to the system because it's being delayed by Apple's incremental rollout mechanism, trying again in ${deferral_timer_minutes} minutes."
				else
					log_super "Warning: The target macOS minor update has not yet automatically downloaded to the system, trying again in ${deferral_timer_minutes} minutes."
					log_status "Pending: The target macOS minor update has not yet automatically downloaded to the system, trying again in ${deferral_timer_minutes} minutes."
				fi
				set_auto_launch_deferral
			else
...
### Woz Custom Display Variables ###
        ...
        # UPDATED Variables for SUPERMAN 5.1 RC2 CONFIRMED 
        wozVersion="2.1 [2/24/26]"
        # 9/26/25 - Added KEV info and fixed 'non-msu' update display for user; Safari, XCode, etc
        # 10/14/25 - ONLY Process SOFAFeed IF major/minor update, ignores for 'non-system' updates now. Cleaned up some scripting
    ...
    if [[ "${macos_msu_major_upgrade_target}" != "FALSE" ]] || [[ "${macos_msu_minor_update_target}" != "FALSE" ]] && [[ "$wozSofaFeedEnabled" -eq 1 ]]; then
        log_super "** Woz SOFA Feed Display: ENABLED **"
        # set trigger for display settings below
        expandedInfo=1
        # MacAdmins SOFA Feed URL [in JSON format!]
        sofaFeed="https://sofafeed.macadmins.io/v1/macos_data_feed.json"
        # SOFA JSON to reference, very hard to parse json through a curl for me
        sofaJSON=/Library/Management/super/sofa.json
    
        # deletes previous sofa.json if found (to keep most up-to-date information)
        if [ -e $sofaJSON ]; then 
            log_super "Woz SOFA Feed: Previous $sofaJSON FOUND!"
            rm -rf "${sofaJSON}"
                log_super "Woz SOFA Feed: Previous $sofaJSON Removed!"
        fi
                    ...
                    sofaCheck=$(/usr/bin/curl --retry 5 --retry-max-time 120 -sf "$sofaFeed" -o "$sofaJSON")
                        wait
                    # check if JSON created/exists to make sure we can proceed
                    if [ -e $sofaJSON ]; then
                        log_super "Woz SOFA JSON: created - $sofaJSON"

                        # gather whole section of the current update (of machine)
                        updateInfoJSON=$(cat "$sofaJSON" | sed 's/.*'"$display_string_workflow_title"'//; s/DaysSincePreviousRelease.*//')
                            #echo "updateInfoJSON var: $updateInfoJSON" # troubleshooting line
        ...
        parsedSecurityInfo=$(echo "$updateInfoJSON" | sed 's/.*SecurityInfo"://; s/,".*//' | tr -d '"')
            ...
            #securityInfo=$(echo "• <a href='"$parsedSecurityInfo"'>Apple Update Info</a>")

        # gather CVE Number information
        parsedCVENum=$(echo "$updateInfoJSON" | sed 's/.*UniqueCVEsCount"://; s/,".*//')
            #echo "parsedCVENum: $parsedCVENum" # troubleshooting line
        log_super "Woz SOFA Display: Total CVE(s) resolved for update/upgrade: $parsedCVENum"
        ...
        if [[ "$parsedCVENum" -lt 5 ]]; then
            log_super "Woz SOFA Display: Less than 5 CVEs ($parsedCVENum). Omiting variable display..."
            cveTitle=""
        # error trying to curl
        elif [[ "$parsedCVENum" == *"Could not extract value"* ]]; then
        ...
        epochRelease=$(/bin/date -j -u -f %Y-%m-%dT%H:%M:%SZ "${parsedReleaseDateInfoZulu}" +%s)
        # converts Epoch time to a nice readable format
        goodDate=$(/bin/date -ur "$epochRelease" "+%A %B %d, %Y")
            #echo "$goodDate" #troubleshooting line

      # Unique CVE (KEVs; Known Exploited Vulnerabilities) count processing
        # created 9/26/25 | UPDATED 12/1/25
        updateInfoJSON=$(cat "$sofaJSON" | sed 's/.*'"$display_string_workflow_title"'//; s/DaysSincePreviousRelease.*//')
        parsedUniqueCVE=$(echo "$updateInfoJSON" | sed 's/.*ActivelyExploitedCVEs"://; s/,".*//' | tr -d '"')
        if [ "$parsedUniqueCVE" -gt 0 ]; then
            KEVs=$(echo "⚠️ **Known Exploited Vulnerabilitie(s) Resolved: $parsedUniqueCVE**")
        else
   log_super "*** Woz SOFA Feed: No Known Exploited Vulnerabilities detected. Omitting KEV variable display! ***"
            KEVs=""
        fi

            # IF the CURL doesn't work, make the display nice so the user doesn't get concerned with failed curl information, lol?
        else
    ...
    # END 'Woz Custom SOFA Feed' processing
    fi

    # checks if there is any 'non-system' [Safari, XCode CLT, etc] updates are available to add to new var display
    if [[ "${non_system_msu_targets}" == "TRUE" ]]; then
                        log_super "Woz Custom Display: Processing Apple 'non-system' updates..."
        for array_index in "${!non_system_msu_array[@]}"; do
            non_system_msu_titles_array+=($(echo "${non_system_msu_array[array_index]}" | awk -F ',' '{print $2}' | sed -e 's/.*://g'))
            otherUpdates=$(echo -e "• $non_system_msu_titles_array")
        done
    else
        # set blank variable to not mess up display settings below
        otherUpdates=""
    fi

    # SAVE FOR FUTURE USE, need to figure out HTML for IBM Notifier
    #<a href=$parsedSecurityInfo>Apple Update Info</a>
        # updated 'IBM Notifier' subtitle info depending on settings
        if [[ "$expandedInfo" -eq 1 ]]; then
            dialogUpdates="**macOS System Update Available** :\n\n• $macOSUpdates\n• Release Date : **$goodDate**\n$cveTitle\n$KEVs\n•More Info: $parsedSecurityInfo\n$otherUpdates"
        else
            dialogUpdates="**macOS Non-System Update(s) Available** :\n\n$otherUpdates\n"
        fi
    fi
...
# This function is designed to run the the background and when killed will output "Active:Inactive:Total" seconds as a string to the file located at input ${1}.
dialog_metrics_counter() {
local dialog_metrics_active_seconds
dialog_metrics_active_seconds=0
local dialog_metrics_inactive_seconds
dialog_metrics_inactive_seconds=0
trap 'echo $dialog_metrics_active_seconds:$dialog_metrics_inactive_seconds:$((dialog_metrics_active_seconds + dialog_metrics_inactive_seconds)) > "${1}"' SIGTERM
while true; do
	if [[ $(($(ioreg -c IOHIDSystem | awk '/HIDIdleTime/ {print $6}') / 1000000000)) -gt 0 ]]; then
		((dialog_metrics_inactive_seconds+=1))
	else
		((dialog_metrics_active_seconds+=1))
	fi
	sleep 1
done
}
...
open_dialog_ibm_notifier() {
	...
	# Start background metrics collection for IBM Notifier dialog.
	log_metrics "Open Dialog $(echo "${dialog_type}" | sed -e 's/ALERT/Interactive Alert/' -e 's/CHOICE/User Choice/' -e 's/SCHEDULE/User Schedule/' -e 's/REMINDER/Schedule Reminder/' -e 's/SOFT/Soft Deadline/' -e 's/AUTH/User Authentication/'): Workflow Elapsed Time" "WorkflowTargetTimestamp"
	local dialog_metrics_counter_fifo
	dialog_metrics_counter_fifo="$(mktemp).fifo"
	mkfifo "${dialog_metrics_counter_fifo}"
	dialog_metrics_counter "${dialog_metrics_counter_fifo}" &
	...
	log_metrics "Close Dialog $(echo "${dialog_type}" | sed -e 's/ALERT/Interactive Alert/' -e 's/CHOICE/User Choice/' -e 's/SCHEDULE/User Schedule/' -e 's/REMINDER/Schedule Reminder/' -e 's/SOFT/Soft Deadline/' -e 's/AUTH/User Authentication/'): User Inactive Elapsed Time" "$(echo "${dialog_metrics_response}" | awk -F ':' '{print $2}')"
	log_metrics "Close Dialog $(echo "${dialog_type}" | sed -e 's/ALERT/Interactive Alert/' -e 's/CHOICE/User Choice/' -e 's/SCHEDULE/User Schedule/' -e 's/REMINDER/Schedule Reminder/' -e 's/SOFT/Soft Deadline/' -e 's/AUTH/User Authentication/'): Dialog Elapsed Time" "$(echo "${dialog_metrics_response}" | awk -F ':' '{print $3}')"
}
...
notification_install_now_start() {
	...
	log_metrics "Open Notification Install Now Start: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_install_now_download() {
	...
	log_metrics "Open Notification Install Now Downloading: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_install_now_up_to_date() {
	...
	log_metrics "Open Notification Install Now Completed: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_install_now_failed() {
	...
	log_metrics "Open Notification Install Now Failed: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_jamf_pro_policy() {
	...
	log_metrics "Open Notification Jamf Pro Policy Installation: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_prepare() {
	...
	log_metrics "Open Notification Preparing Update/Upgrade: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_restart() {
	...
	log_metrics "Open Notification Restart: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_non_system_updates() {
	...
	log_metrics "Open Notification Installing Non-System Updates: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_safari_update() {
	...
	log_metrics "Open Notification Installing Safari Update: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
notification_failed() {
	...
	log_metrics "Open Notification Installation Failed: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
dialog_insufficient_storage() {
	...
	# Manage ${display_unmovable}, ${display_hide_background}, ${display_silently}, and ${display_hide_progress_bar}.
	...
	# Reset temporary ${display_unmovable}, ${display_hide_background}, ${display_silently}, and ${display_hide_progress_bar}.
	...
	if [[ "${dialog_insufficient_storage_error}" == "FALSE" ]] && [[ "${storage_ready}" == "FALSE" ]]; then
		...
		dialog_insufficient_storage_error="TRUE"
		log_metrics "Dialog Insufficient Storage Timeout: Workflow Elapsed Time" "WorkflowTargetTimestamp"
	fi
...
dialog_power_required() {
	...
	while [[ $dialog_power_required_check_timeout -ge 0 ]]; do
		...
		if [[ "${power_required_charger_connected}" == "TRUE" ]]; then
			...
			killall -9 "IBM Notifier" "IBM Notifier Popup" >/dev/null 2>&1
			log_metrics "Dialog Power Required Success: Workflow Elapsed Time" "WorkflowTargetTimestamp"
			break
		fi
	...
	if [[ "${power_required_charger_connected}" == "FALSE" ]]; then
		...
		log_metrics "Dialog Power Required Timeout: Workflow Elapsed Time" "WorkflowTargetTimestamp"
	fi
...
dialog_user_choice() {
	...
	0)
		...
		log_metrics "Dialog User Choice Deferral: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		set_auto_launch_deferral
		;;
	2)
		log_super "Status: User chose to install now."
		log_metrics "Dialog User Choice Installation: Workflow Elapsed Time" "WorkflowTargetTimestamp"
	...
	3)
		...
		log_metrics "Dialog User Choice Schedule Installation: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
dialog_user_schedule() {
	...
	0)
		...
		log_metrics "Dialog User Schedule Set: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		if [[ -n "${schedule_workflow_active}" ]]; then
			set_schedule_workflow_active_adjustments
			if [[ -n "${workflow_scheduled_install_adjusted}" ]]; then
				workflow_scheduled_install="${workflow_scheduled_install_adjusted}"
	...
	2)
		...
		log_metrics "Dialog User Schedule Cancel: Workflow Elapsed Time" "WorkflowTargetTimestamp"
...
dialog_schedule_reminder() {
	...
	if [[ "${scheduled_install_final_reminder}" == "TRUE" ]]; then
		...
		0)
			log_super "Status: User chose to install now."
			log_metrics "Dialog Schedule Reminder User Installation: Workflow Elapsed Time" "WorkflowTargetTimestamp"
			;;
		2)
			delete_pref_main "WorkflowScheduledInstall"
			log_super "Status: User chose to reschedule, removing previously scheduled installation and restarting super..."
			log_metrics "Dialog Schedule Reminder User Reschedule: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		...
		0)
			...
			log_metrics "Dialog Schedule Reminder User Dimsissed: Workflow Elapsed Time" "WorkflowTargetTimestamp"
			;;
		2)
			delete_pref_main "WorkflowScheduledInstall"
...
dialog_soft_deadline() {
	...
	0)
		...
		log_metrics "Dialog Soft Deadline User Installation: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		;;
	4 | 137 | 255)
...
dialog_user_auth() {
	...
	if [[ "${dialog_user_auth_valid}" == "TRUE" ]]; then
		...
		log_metrics "Dialog User Authentication Successful: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		if [[ "${auth_ask_user_to_save_password}" == "TRUE" ]]; then
		...
		if [[ "${dialog_user_auth_timeout}" == "TRUE" ]]; then
			...
			log_metrics "Dialog User Authentication Timeout: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		else
			log_super "Error: User authentication failed."
			log_metrics "Dialog User Authentication Failed: Workflow Elapsed Time" "WorkflowTargetTimestamp"
		fi
		dialog_user_auth_error="TRUE"
	fi
...
main() {
	...
	if [[ "${workflow_disable_update_check}" == "TRUE" ]]; then # Skip software updates/upgrade mode option.
		...
		if [[ "${test_mode}" != "TRUE" ]]; then # Default workflow starts by checking for updates/upgrades.
			...
			elif [[ "${install_safari_updates_without_restarting}" == "TRUE" ]]; then
				non_system_msu_targets="TRUE"
	...
	if [[ "${install_prioritize_non_restart_updates}" == "TRUE" ]] && [[ "${non_system_msu_targets}" == "TRUE" ]]; then
		...
		macos_msu_major_upgrade_target="FALSE"
		macos_msu_minor_update_target="FALSE"
		if [[ "${install_safari_updates_without_restarting}" == "TRUE" ]] && [[ "${non_system_msu_safari_target}" != "FALSE" ]]; then
			...
			[[ "${install_non_system_updates_without_restarting}" == "TRUE" ]] && log_super "Warning: The --install-safari-updates-without-restarting option is currently overriding the --install-non-system-updates-without-restarting option."
			[[ "${workflow_only_download_active}" == "TRUE" ]] && log_super "Warning: The --install-safari-updates-without-restarting option is currently overriding the --only-download option."
		elif [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; then
		...
		if [[ "${non_system_msu_targets}" == "TRUE" ]]; then
			log_super "Status: Non-system updates will be installed immediately after the system restarts from the pending macOS major upgrade."
			if [[ "${install_safari_updates_without_restarting}" == "TRUE" ]] && [[ "${non_system_msu_safari_target}" != "FALSE" ]]; then
				log_super "Warning: To force Safari updates to install before a macOS update/upgrade then you must also enable the --install-prioritize-non-restart-updates option."
			elif [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; then
				log_super "Warning: To force non-system updates to install before a macOS update/upgrade then you must also enable the --install-prioritize-non-restart-updates option."
			fi
		fi
		...
		[[ -n "${display_accessory_macos_minor_update}" ]] && display_accessory_content="${display_accessory_macos_minor_update}"
		if [[ "${non_system_msu_targets}" == "TRUE" ]]; then
			log_super "Status: Non-system updates will be installed immediately after the system restarts from the pending macOS minor update."
			if [[ "${install_safari_updates_without_restarting}" == "TRUE" ]] && [[ "${non_system_msu_safari_target}" != "FALSE" ]]; then
				log_super "Warning: To force Safari updates to install before a macOS update/upgrade then you must also enable the --install-prioritize-non-restart-updates option."
			elif [[ "${install_non_system_updates_without_restarting}" == "TRUE" ]]; then
				log_super "Warning: To force non-system updates to install before a macOS update/upgrade then you must also enable the --install-prioritize-non-restart-updates option."
	...
	# Log any changes regarding the workflow target.
	...
	if [[ "${workflow_target}" != "FALSE" ]] && [[ -z "${workflow_target_previous}" ]]; then
		log_super_audit "Status: Setting new workflow target to ${workflow_target}".
		log_metrics "New Workflow Target"
		set_pref_main "WorkflowTargetTimestamp" "$(date -r "${workflow_time_epoch}" +%Y-%m-%d:%H:%M:%S)"
	fi
	if [[ "${workflow_target}" != "FALSE" ]] && [[ -n "${workflow_target_previous}" ]] && [[ "${workflow_target}" != "${workflow_target_previous}" ]]; then
		log_super_audit "Warning: Previous workflow target of ${workflow_target_previous} has been changed to ${workflow_target}."
		log_metrics "Replaced Previous Workflow Target"
		set_pref_main "WorkflowTargetTimestamp" "$(date -r "${workflow_time_epoch}" +%Y-%m-%d:%H:%M:%S)"
	fi
		...
		if [[ "${workflow_install_now_active}" == "TRUE" ]]; then
			...
			if [[ "${current_user_account_name}" == "FALSE" ]]; then # A normal user is not logged in, start installations immediately.
				...
				if [[ "${deadline_date_status}" == "HARD" ]] || [[ "${deadline_days_status}" == "HARD" ]] || [[ "${deadline_count_status}" == "HARD" ]] || [[ "${deadline_date_status}" == "SOFT" ]] || [[ "${deadline_days_status}" == "SOFT" ]] || [[ "${deadline_count_status}" == "SOFT" ]]; then
					...
					[[ $(echo "${display_hide_progress_bar}" | grep -c 'DEADLINE') -gt 0 ]] && display_hide_progress_bar_status="TRUE"
